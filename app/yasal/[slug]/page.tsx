    ...
    <main className="min-h-screen px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="glass-card rounded-[1.75rem] p-6 sm:p-8">
          ...
          <p className="mt-3 flex items-center gap-1.5 text-xs text-mist/40">
            ...
            {page.sections.map((section, idx) => (
              <section key={idx}>
                ...
                {section.paragraphs?.map((p, pIdx) => (
                  <p key={pIdx} className="mb-2 text-sm leading-6 text-mist/68">{p}</p>
                ))}
                {section.bullets ? (
                  <ul className="space-y-2">
                    {section.bullets.map((item, iIdx) => (
                      <li key={iIdx} className="flex items-start gap-2.5 text-sm leading-6 text-mist/68">
