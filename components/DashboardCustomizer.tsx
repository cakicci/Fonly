    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div className="relative w-full max-w-md rounded-3xl border border-line bg-surface p-5 shadow-glow">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-base font-semibold text-mist">Paneli özelleştir</h2>
                <p className="mt-0.5 text-xs text-mist-3">Kartları gizle veya sırasını değiştir.</p>
              </div>
              <button
                onClick={() => setOpen(false)}
                aria-label="Kapat"
                className="rounded-lg p-1.5 text-mist-3 transition hover:bg-white/10 hover:text-mist"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <ul className="mt-4 space-y-2">
              {items.map((it, i) => (
                <li
                  key={it.key}
                  className="flex items-center gap-2 rounded-xl border border-line bg-white/[0.03] px-3 py-2"
                >
                  <button
                    onClick={() => toggle(i)}
                  ...
                  <span
                    className={`flex-1 truncate text-sm ${
                      it.visible ? "text-mist" : "text-mist-3 line-through"
                    }`}
                  >
                    {it.label}
                  </span>
                  <button
                    ...
                    className="rounded-lg p-1.5 text-mist-3 transition hover:bg-white/10 hover:text-mist disabled:opacity-25"
                  >
            ...
            <div className="mt-5 flex items-center justify-between gap-3">
              <button
                onClick={reset}
                className="inline-flex items-center gap-1.5 text-xs font-semibold text-mist-3 transition hover:text-mist"
              >
