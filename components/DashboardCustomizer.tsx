          ...
          <div className="relative w-full max-w-md rounded-3xl border border-white/10 bg-surface p-5 shadow-glow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-mist/50">Kartları gizle veya sırasını değiştir.</p>
              </div>
              <button
                onClick={() => setOpen(false)}
                aria-label="Kapat"
                className="rounded-lg p-1.5 text-mist/50 transition hover:bg-white/10 hover:text-white"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
                ...
                >
                  <button
                    ...
                    className={`rounded-lg p-1.5 transition ${
                      it.visible
                        ...
                        : "text-mist/35 hover:bg-white/10"
                    }`}
                  >
                  ...
                  <span
                    className={`flex-1 truncate text-sm ${
                      it.visible ? "text-white" : "text-mist/40 line-through"
                    }`}
                  >
                    {it.label}
                  </span>
                  <button
                    ...
                    className="rounded-lg p-1.5 text-mist/55 transition hover:bg-white/10 hover:text-white disabled:opacity-25"
                  >
            ...
            <div className="mt-5 flex items-center justify-between gap-3">
              <button
                onClick={reset}
                className="inline-flex items-center gap-1.5 text-xs font-semibold text-mist/60 transition hover:text-white"
              >
              ...
              <button
                onClick={save}
                disabled={saving}
                className="inline-flex items-center gap-2 rounded-xl bg-emerald-300 px-5 py-2.5 text-sm font-semibold text-ink transition hover:bg-emerald-200 disabled:opacity-60"
              >
