...
const COLOR_VALUE = "#0eaf7b";
const COLOR_COST = "#8b5cf6";
...
export function PortfolioValueChart() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch(`/api/portfolio/history?range=${range}`, { cache: "no-store" })
  ...
  return (
    <section className="glass-card rounded-2xl p-5 ring-1 ring-white/8">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex rounded-xl border border-white/10 p-0.5" role="group" aria-label="Zaman aralığı">
          ...
          <div className="h-56 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={points} margin={{ top: 4, right: 4, bottom: 0, left: 4 }}>
                ...
                <CartesianGrid stroke="rgba(216,247,238,0.06)" vertical={false} />
                <XAxis
                  dataKey="date"
                  tickFormatter={tickDate}
                  tick={{ fill: "rgba(216,247,238,0.4)", fontSize: 11 }}
                  axisLine={false}
                  tickLine={false}
                  minTickGap={48}
                />
                ...
                <Tooltip
                  ...
                  cursor={{ stroke: "rgba(216,247,238,0.2)", strokeWidth: 1 }}
                />
                <Area
                  type="monotone"
                  dataKey="value"
