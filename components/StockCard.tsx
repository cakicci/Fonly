...
export function StockCard({ stock }: StockCardProps) {
  ...
  return (
    <article className="glass-card h-full rounded-[1.25rem] p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-mist/52">Geçmiş performans özeti</p>
        </div>
        <div
          className={
            isPositive
      ...
      <div className="mt-6 flex items-end justify-between gap-4 border-t border-white/8 pt-5">
        <div>
          <p className="text-xs text-mist/54">Fiyat</p>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl border border-white/8 bg-white/[0.04] p-3">
          <p className="text-xs text-mist/54">5 yılda hisse</p>
          <p className="mt-1 text-xl font-semibold text-emerald-200">{stock.fiveYearReturn}</p>
        </div>
        <div className="rounded-2xl border border-white/8 bg-white/[0.04] p-3">
          <p className="text-xs text-mist/54">Altınla kıyas</p>
          <p className="mt-1 text-sm leading-5 text-mist/76">{stock.goldComparison}</p>
        </div>
      </div>
      ...
      <p className="mt-5 text-sm leading-6 text-mist/66">{stock.explanation}</p>
    </article>
  );
}
